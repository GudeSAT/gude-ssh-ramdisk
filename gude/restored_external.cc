...
int main(){
    sleep(8);
    printf("****************************************************\n");
    printf("****************   goodsat.cn  @gude    ************\n");
    printf("*************Starting inetd and USB magic***********\n");
    printf("**************** HA HA HA HA HA HA HA **************\n");
    printf("****************************************************\n");
    int pid, i;
    char *arg[] = {"mount_apfs", "/dev/disk0s1", "/mnt1", NULL}; // rootfs, may be empty or not. We attempt all this to stop restored_external from trying it itself and rebooting when it fucks up
    ...
    char *arg5[] = {"bash","-c","/System/Library/Filesystems/apfs.fs/mount_apfs /dev/disk0s2 /mnt2", NULL}; // /private/var is mounted on this one
    ...
    char *arg8[] = {"launchctl", "load", "-w", "/Library/LaunchDaemons/com.openssh.sshd.plist", NULL};
    posix_spawn(&pid, "/bin/launchctl", NULL, NULL, (char* const*)arg8, NULL);
    waitpid(pid, &i, 0);
    sleep(99999);
  return 0;
}
